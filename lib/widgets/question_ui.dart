...
class QuestionUI extends StatefulWidget {
  const QuestionUI({super.key, required this.quizSession, required this.index, required this.pageC});
    ...
    if (question is Fitb) {
      ...
      for (int i = 0; i < fitb.question.split(' ').length; i++) {
        ...
        if (fitbWord == '#fitb') {
          ...
          if (widget.quizSession.selectedAnswers[widget.index].length < noOfBlanks) {
            widget.quizSession.selectedAnswers[widget.index].add('');
          }

          fitbBlankCs
              .add(TextEditingController(text: widget.quizSession.selectedAnswers[widget.index][noOfBlanks - 1]));
        } else {
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  itemCount: question is Mcq ? mcq.options.length : fitbBlankCs.length,
                    ...
                    return Padding(
                      ...
                      child: InkWell(
                        onTap: () => question is Mcq
                            ? setState(() {
                                if (!widget.quizSession.selectedAnswers[widget.index].contains(index)) {
                                  if (mcq.answerIndex.length == 1) {
                                    widget.quizSession.selectedAnswers[widget.index].clear();
                                  }
                                  widget.quizSession.selectedAnswers[widget.index].add(index);
                                } else {
                                  if (mcq.answerIndex.length == 1) {
                                    widget.quizSession.selectedAnswers[widget.index].clear();
                                  } else {
                                    widget.quizSession.selectedAnswers[widget.index].remove(index);
                                  }
                                }
                              })
                            : null,
                        child: Container(
                          decoration: BoxDecoration(
                              ...
                              color: widget.quizSession.selectedAnswers[widget.index].contains(index)
                              ...
                              ? Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      if (mcq.answerIndex.length != 1)
                                        widget.quizSession.selectedAnswers[widget.index].contains(index)
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: TextFormField(
                                          ...
                                          onChanged: (value) {
                                            if (widget.quizSession.selectedAnswers[widget.index].length >= index + 1) {
                                              widget.quizSession.selectedAnswers[widget.index][index] =
                                                  value.toLowerCase();
                                            } else {
                                              widget.quizSession.selectedAnswers[widget.index].add(value.toLowerCase());
                                            }
                                          },
                                        ),
                                      ),
          ...
          Align(
            ...
            child: Padding(
              ...
              child: IconButton(
                  onPressed: () {
                    widget.pageC
                        .previousPage(duration: const Duration(milliseconds: 650), curve: Curves.linearToEaseOut);
                  },
        ...
        if (widget.index + 1 != widget.quizSession.questions.length)
          Align(
            ...
            child: Padding(
              ...
              child: IconButton(
                  onPressed: () {
                    widget.pageC.nextPage(duration: const Duration(milliseconds: 650), curve: Curves.linearToEaseOut);
                  },
